...
class ServerlessAppsyncPlugin {
  ...
  generateDbClusterArn(dbClusterIdentifier, region) {
    return {
      'Fn::Join': [
        ...
        [
          'arn',
          identifyServerLocationByRegion(region, 'aws-cn', 'aws'),
          'rds',
          region,
          { Ref: 'AWS::AccountId' },
          'cluster',
    ...
    return {
      [logicalIdGraphQLApi]: {
        ...
        Properties: {
          ...
          AdditionalAuthenticationProviders: config.additionalAuthenticationProviders.map(
            (provider) =>
        ...
        config.logConfig.level && {
          [`${logicalIdGraphQLApi}LogGroup`]: {
            ...
            Properties: {
              ...
              RetentionInDays: this.serverless.service.provider
                .logRetentionInDays,
            },
          },
        }),
      ...
      ? {
          [`${logicalIdGraphQLApi}LambdaAuthorizerPermission`]: {
            ...
            Properties: {
              ...
              Principal: identifyServerLocationByRegion(
                config.region,
                'appsync.cn-north-1.amazonaws.com.cn',
            ...
            Statement: [
              {
                ...
                Principal: {
                  Service: [
                    identifyServerLocationByRegion(
                      config.region,
                      'appsync.cn-north-1.amazonaws.com.cn',
                      'appsync.amazonaws.com',
                    ),
      ...
      case 'AMAZON_DYNAMODB': {
        const dynamoDbResourceArn = {
          'Fn::Join': [
            ...
            [
              'arn',
              identifyServerLocationByRegion(config.region, 'aws-cn', 'aws'),
              'dynamodb',
    ...
    return config.dataSources.reduce((acc, ds) => {
      ...
      if (ds.type === 'AWS_LAMBDA') {
        ...
        if (resource.Properties.DynamoDBConfig.Versioned) {
          resource.Properties.DynamoDBConfig.DeltaSyncConfig = this.getDeltaSyncConfig(
            Object.assign({}, ds.config),
          );
        }
  ...
  getFunctionConfigurationResources(config) {
    const flattenedFunctionConfigurationResources = config.functionConfigurations.reduce(
...
function identifyServerLocationByRegion(region, val, defaults) {
  const outsideGlobal = ['cn-north-1', 'cn-northwest-1'];
  const nonGlobal = outsideGlobal.includes(region);
  return nonGlobal ? val : defaults;
}
